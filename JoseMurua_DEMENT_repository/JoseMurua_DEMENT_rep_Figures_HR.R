...
#     Processing the data from simulations with high redundancy
#     ------------------------------------
...
# Including only simulations with high redundancy
all.Data <- all.Data[str_detect(all.Data, "02.R")]
...
write_tsv(Biomass.df, "Biomass_HR.txt")
...
if(ncol(MicrobesSeries[[1]]) > 3){
  ...
  write_tsv(Taxa.df, "Taxa_HR.txt"); write_tsv(Taxa.df.l, "TaxaL_HR.txt"); write_tsv(TaxaGenes, "TaxaGenes_HR.txt")
...
write_tsv(Growth.df, "Growth_HR.txt")
...
Respiration.df <- as.data.frame(RespSeries) %>%  cbind(days = 1:length(RespSeries[[1]])) %>%
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Respiration.df, "Respiration_HR.txt")
...
Litter.df <- as.data.frame(Litter) %>%  cbind(days = 1:length(Litter[[1]])) %>%
  pivot_longer(cols = 1:length(Litter), names_to = "Simulation", 
               values_to = "Litter") %>% 
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Litter.df, "Litter_HR.txt")
...
write_tsv(Litter.subs.df, "LitterSubs_HR.txt")
...
write_tsv(Litter.subsN.df, "LitterSubsN_HR.txt")
...
write_tsv(Litter.subsP.df, "LitterSubsP_HR.txt")
...
write_tsv(Litter.subs.df.l, "LitterSubsL_HR.txt")
...
write_tsv(C.MonSer.df, "CMonSer_HR.txt")
...
C.MonSerLitt.df$C.MonSerLitt <- 
  rowSums(C.MonSer.df[, c("Cellulose", "Hemicellulose", "Starch", "Chitin", 
                          "Lignin", "Protein1", "Protein2", 
                          "Protein3", "OrgP1", "OrgP2")])
write_tsv(C.MonSerLitt.df, "CMonSerLitt_HR.txt")
...
N.MonSer.df$N.MonSer <- rowSums(N.MonSer.df[, c("NH4", "DeadMic", "DeadEnz", "Cellulose", 
                                                ...
                                                "Lignin", "Protein1", "Protein2", 
                                                "Protein3", "OrgP1", "OrgP2")])
write_tsv(N.MonSer.df, "NMonSer_HR.txt")
...
N.MonSerLitt.df$N.MonSerLitt <- rowSums(N.MonSer.df[, c("Cellulose", 
                                                        ...
                                                        "Lignin", "Protein1", "Protein2", 
                                                        "Protein3", "OrgP1", "OrgP2")])
write_tsv(N.MonSerLitt.df, "NMonSerLitt_HR.txt")
...
write_tsv(MonSer.df, "MonSer_HR.txt")
...
write_tsv(MonSerLitt.df, "MonSerLitt_HR.txt")
...
write_tsv(NutSer.df, "NutSer_HR.txt"); write_tsv(NutSer.df.l, "NutSerL_HR.txt")
...
write_tsv(MineralSer.df, "MineralSer_HR.txt")
...
write_tsv(MonProdSer_CN.df, "MonProdSerCN_HR.txt")
...
write_tsv(EnzSer.df, "EnzSer_HR.txt")
...
write_tsv(EnzSer.df.l, "EnzSerL_HR.txt")
...
write_tsv(EnzProdSer.df.l, "EnzProdSerL_HR.txt")
...
UptSer.df <- inner_join(C.UptSer.df, N.UptSer.df) %>% inner_join(P.UptSer.df) %>% 
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(UptSer.df, "UptSer_HR.txt")
...
write_tsv(N.MonomerUptake.df, "N_MonomerUptake_HR.txt")
...
Net.CUE.df <- as.data.frame(Net.CUE_Series) %>%  cbind(days = 1:1000) %>%
  pivot_longer(cols = 1:length(Net.CUE_Series), names_to = "Simulation", 
               values_to = "CUE") %>% 
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Net.CUE.df, "NetCUE_HR.txt")
...
write_tsv(Tot.growth, "Tot_Growth_HR.txt")
...
write_tsv(Tot.resp, "TotResp_HR.txt")

Tot.resp.comp <- 
  Resp.comp.df %>% 
  group_by(Simulation, Mechanism, Litter.CN, Source) %>%
  summarize(Tot.resp = sum(Resp, na.rm = TRUE))
write_tsv(Tot.resp.comp, "TotRespComp_HR.txt")
...
write_tsv(Tot.C.Uptake, "TotCuptake_HR.txt")
...
write_tsv(Tot.CUE, "TotCUE_HR.txt")
...
Tot.Biomass <-
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Tot.Biomass, "TotBiomass_HR.txt")
...
Tot.Enz <- 
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Tot.Enz, "TotEnz_HR.txt")
...
Tot.Mon <- 
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Tot.Mon, "TotMon_HR.txt")
...
write_tsv(Tot.mass.loss, "TotMassLoss_HR.txt")

Days.to.50 <- 
  Litter.subs.df %>% group_by(Simulation, Mechanism, Litter.CN) %>%
  mutate(Remaining = (Litter.C/max(Litter.C, na.rm = TRUE))*100) %>%
  filter(Remaining > 50) %>% summarize(Days.to.50 = max(days, na.rm = TRUE))
write_tsv(Days.to.50, "DaysTo50_HR.txt")
...
write_tsv(Tot.excretion, "TotExcretion_HR.txt")
...
# 
# # Total Biomass
# pdf("Total_Biomass_HR.pdf", height = 3, width = 6)
...
# pdf("Total_Carbon_Growth_HR.pdf", height = 3, width = 6)
...
# 
# # Total Respiration
# pdf("Total_respiration_HR.pdf", height = 3, width = 6)
...
# pdf("Total_respiration_per_biomass_HR.pdf", height = 3, width = 6)
...
# pdf("Overflow_respiration_HR.pdf", height = 3, width = 6)
...
# pdf("Maintenance_respiration_HR.pdf", height = 3, width = 6)
# Tot.resp.comp %>% mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   filter(Source == "Maint") %>%
#   ggplot(aes(x = Litter.CN, y = Tot.resp)) +
#   geom_path(aes(color = Mechanism)) +
...
# pdf("Maintenance_per_biomass_HR.pdf", height = 3, width = 6)
...
# 
# # Growth respiration
# pdf("Growth_respiration_HR.pdf", height = 3, width = 6)
# Tot.resp.comp %>% mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
...
# pdf("Growth_respiration_per_biomass_HR.pdf", height = 3, width = 6)
# Tot.resp.comp %>% left_join(Tot.Biomass) %>%
#   mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   filter(Source == "Growth") %>%
#   ggplot(aes(x = Litter.CN, y = Tot.resp/Tot.Biomass)) +
...
# pdf("CUE_HR.pdf", height = 3, width = 6)
# Tot.CUE %>% mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
...
# pdf("Years_to_50_mass_loss_HR.pdf", height = 3, width = 6)
...
# pdf("Final_remaining_mass_HR.pdf", height = 3, width = 6)
...
# 
# # Total excreted NH4
# pdf("Excreted_NH4_HR.pdf", height = 3, width = 6)
...
# 
# # Total excreted PO4
# pdf("Excreted_PO4_HR.pdf", height = 3, width = 6)
# Tot.excretion %>% mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
...
# pdf("Enzyme_CN_HR.pdf", height = 3, width = 6)

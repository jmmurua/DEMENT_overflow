...
#     Processing the data from simulations with low redundancy
#     ------------------------------------
...
# Including only simulations with low redundancy
all.Data <- all.Data[str_detect(all.Data, "01.R")]
...
write_tsv(Biomass.df, "Biomass_LR.txt")
...
if(ncol(MicrobesSeries[[1]]) > 3){
  ...
  write_tsv(Taxa.df, "Taxa_LR.txt"); write_tsv(Taxa.df.l, "TaxaL_LR.txt"); write_tsv(TaxaGenes, "TaxaGenes_LR.txt")
...
write_tsv(Growth.df, "Growth_LR.txt")
...
Respiration.df <- as.data.frame(RespSeries) %>%  cbind(days = 1:length(RespSeries[[1]])) %>%
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Respiration.df, "Respiration_LR.txt")
...
Litter.df <- as.data.frame(Litter) %>%  cbind(days = 1:length(Litter[[1]])) %>%
  pivot_longer(cols = 1:length(Litter), names_to = "Simulation", 
               values_to = "Litter") %>% 
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Litter.df, "Litter_LR.txt")
...
write_tsv(Litter.subs.df, "LitterSubs_LR.txt")
...
write_tsv(Litter.subsN.df, "LitterSubsN_LR.txt")
...
write_tsv(Litter.subsP.df, "LitterSubsP_LR.txt")
...
write_tsv(Litter.subs.df.l, "LitterSubsL_LR.txt")
...
write_tsv(C.MonSer.df, "CMonSer_LR.txt")
...
C.MonSerLitt.df$C.MonSerLitt <- 
  rowSums(C.MonSer.df[, c("Cellulose", "Hemicellulose", "Starch", "Chitin", 
                          "Lignin", "Protein1", "Protein2", 
                          "Protein3", "OrgP1", "OrgP2")])
write_tsv(C.MonSerLitt.df, "CMonSerLitt_LR.txt")
...
N.MonSer.df$N.MonSer <- rowSums(N.MonSer.df[, c("NH4", "DeadMic", "DeadEnz", "Cellulose", 
                                                ...
                                                "Lignin", "Protein1", "Protein2", 
                                                "Protein3", "OrgP1", "OrgP2")])
write_tsv(N.MonSer.df, "NMonSer_LR.txt")
...
N.MonSerLitt.df$N.MonSerLitt <- rowSums(N.MonSer.df[, c("Cellulose", 
                                                        ...
                                                        "Lignin", "Protein1", "Protein2", 
                                                        "Protein3", "OrgP1", "OrgP2")])
write_tsv(N.MonSerLitt.df, "NMonSerLitt_LR.txt")
...
write_tsv(MonSer.df, "MonSer_LR.txt")
...
write_tsv(MonSerLitt.df, "MonSerLitt_LR.txt")
...
write_tsv(NutSer.df, "NutSer_LR.txt"); write_tsv(NutSer.df.l, "NutSerL_LR.txt")
...
write_tsv(MineralSer.df, "MineralSer_LR.txt")
...
write_tsv(MonProdSer_CN.df, "MonProdSerCN_LR.txt")
...
write_tsv(EnzSer.df, "EnzSer_LR.txt")
...
write_tsv(EnzSer.df.l, "EnzSerL_LR.txt")
...
write_tsv(EnzProdSer.df.l, "EnzProdSerL_LR.txt")
...
UptSer.df <- inner_join(C.UptSer.df, N.UptSer.df) %>% inner_join(P.UptSer.df) %>% 
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(UptSer.df, "UptSer_LR.txt")
...
write_tsv(N.MonomerUptake.df, "N_MonomerUptake_LR.txt")
...
Net.CUE.df <- as.data.frame(Net.CUE_Series) %>%  cbind(days = 1:1000) %>%
  pivot_longer(cols = 1:length(Net.CUE_Series), names_to = "Simulation", 
               values_to = "CUE") %>% 
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Net.CUE.df, "NetCUE_LR.txt")
...
write_tsv(Tot.growth, "Tot_Growth_LR.txt")
...
write_tsv(Tot.resp, "TotResp_LR.txt")

Tot.resp.comp <- 
  Resp.comp.df %>% 
  group_by(Simulation, Mechanism, Litter.CN, Source) %>%
  summarize(Tot.resp = sum(Resp, na.rm = TRUE))
write_tsv(Tot.resp.comp, "TotRespComp_LR.txt")
...
write_tsv(Tot.C.Uptake, "TotCuptake_LR.txt")
...
write_tsv(Tot.CUE, "TotCUE_LR.txt")
...
Tot.Biomass <-
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Tot.Biomass, "TotBiomass_LR.txt")
...
Tot.Enz <- 
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Tot.Enz, "TotEnz_LR.txt")
...
Tot.Mon <- 
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Tot.Mon, "TotMon_LR.txt")
...
write_tsv(Tot.mass.loss, "TotMassLoss_LR.txt")

Days.to.50 <- 
  Litter.subs.df %>% group_by(Simulation, Mechanism, Litter.CN) %>%
  mutate(Remaining = (Litter.C/max(Litter.C, na.rm = TRUE))*100) %>%
  filter(Remaining > 50) %>% summarize(Days.to.50 = max(days, na.rm = TRUE))
write_tsv(Days.to.50, "DaysTo50_LR.txt")
...
write_tsv(Tot.excretion, "TotExcretion_LR.txt")
...
# # Total Biomass
# pdf("Total_Biomass_LR.pdf", height = 4, width = 5)
...
# pdf("Total_Carbon_Growth_LR.pdf", height = 3, width = 6)
...
# 
# # Total Respiration
# pdf("Total_respiration_LR.pdf", height = 3, width = 6)
...
# pdf("Total_respiration_per_biomass_LR.pdf", height = 3, width = 6)
...
# pdf("Overflow_respiration_LR.pdf", height = 4, width = 5)
...
# pdf("Maintenance_respiration_LR.pdf", height = 4, width = 5)
# Tot.resp.comp %>% #mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   #filter(Source == "Maint") %>%
...
# pdf("Maintenance_per_biomass_LR.pdf", height = 3, width = 6)
...
# 
# # Growth respiration
# pdf("Growth_respiration_LR.pdf", height = 4, width = 5)
# Tot.resp.comp %>% #mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
...
# pdf("Growth_respiration_per_biomass_LR.pdf", height = 3, width = 6)
# Tot.resp.comp %>% left_join(Tot.Biomass) %>%
#   mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   filter(Source == "Growth") %>%
#   ggplot(aes(x = Litter.CN, y = Tot.resp/Tot.Biomass)) +
...
# pdf("CUE_LR.pdf", height = 4, width = 5)
# Tot.CUE %>% #mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
...
# pdf("Years_to_50_mass_loss_LR.pdf", height = 4, width = 5)
...
# pdf("Final_remaining_mass_LR.pdf", height = 3, width = 6)
...
# 
# # Total excreted NH4
# pdf("Total_NH4_excreted_LR.pdf", height = 4, width = 5)
# Tot.mineral %>% 
...
# 
# # Total excreted PO4
# pdf("Excreted_PO4_LR.pdf", height = 3, width = 6)
...
# pdf("Enzyme_CN_LR.pdf", height = 4, width = 5)
...
# 
# # Final litter C:N
# pdf("FinalLitterCN_LR.pdf", height = 4, width = 5)
# Final.Litter %>%
...
# pdf("TotMonProdCN_LR.pdf", height = 4, width = 5)
# Tot.monProdSer.CN %>%
...
# 
# # Uptake C:N
# pdf("UptakeCN_LR.pdf", height = 4, width = 5)
# Tot.uptake %>%
...
# 
# # Biomass C:N
# pdf("BiomassCN_LR.pdf", height = 4, width = 5)
# Biomass.CN %>%
...
# pdf("NUE_LR.pdf", height = 4, width = 5)
# Tot.growth %>% 

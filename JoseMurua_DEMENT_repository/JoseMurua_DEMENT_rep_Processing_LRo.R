...
#     Processing the data from simulations with low redundancy (DOC overflow)
#     ------------------------------------
...
# Including only simulations with low redundancy and DOC overflow
all.Data <- all.Data[str_detect(all.Data, "04.R")]
...
write_tsv(Biomass.df, "Biomass_LRo.txt")
...
if(ncol(MicrobesSeries[[1]]) > 3){
  ...
  write_tsv(Taxa.df, "Taxa_LRo.txt"); write_tsv(Taxa.df.l, "TaxaL_LRo.txt"); write_tsv(TaxaGenes, "TaxaGenes_LRo.txt")
...
write_tsv(Growth.df, "Growth_LRo.txt")
...
Respiration.df <- as.data.frame(RespSeries) %>%  cbind(days = 1:length(RespSeries[[1]])) %>%
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Respiration.df, "Respiration_LRo.txt")
...
Litter.df <- as.data.frame(Litter) %>%  cbind(days = 1:length(Litter[[1]])) %>%
  pivot_longer(cols = 1:length(Litter), names_to = "Simulation", 
               values_to = "Litter") %>% 
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Litter.df, "Litter_LRo.txt")
...
write_tsv(Litter.subs.df, "LitterSubs_LRo.txt")
...
write_tsv(Litter.subsN.df, "LitterSubsN_LRo.txt")
...
write_tsv(Litter.subsP.df, "LitterSubsP_LRo.txt")
...
write_tsv(Litter.subs.df.l, "LitterSubsL_LRo.txt")
...
write_tsv(C.MonSer.df, "CMonSer_LRo.txt")
...
C.MonSerLitt.df$C.MonSerLitt <- 
  rowSums(C.MonSer.df[, c("Cellulose", "Hemicellulose", "Starch", "Chitin", 
                          "Lignin", "Protein1", "Protein2", 
                          "Protein3", "OrgP1", "OrgP2")])
write_tsv(C.MonSerLitt.df, "CMonSerLitt_LRo.txt")
...
N.MonSer.df$N.MonSer <- rowSums(N.MonSer.df[, c("NH4", "DeadMic", "DeadEnz", "Cellulose", 
                                                ...
                                                "Lignin", "Protein1", "Protein2", 
                                                "Protein3", "OrgP1", "OrgP2")])
write_tsv(N.MonSer.df, "NMonSer_LRo.txt")
...
N.MonSerLitt.df$N.MonSerLitt <- rowSums(N.MonSer.df[, c("Cellulose", 
                                                        ...
                                                        "Lignin", "Protein1", "Protein2", 
                                                        "Protein3", "OrgP1", "OrgP2")])
write_tsv(N.MonSerLitt.df, "NMonSerLitt_LRo.txt")
...
write_tsv(MonSer.df, "MonSer_LRo.txt")
...
write_tsv(MonSerLitt.df, "MonSerLitt_LRo.txt")
...
write_tsv(NutSer.df, "NutSer_LRo.txt"); write_tsv(NutSer.df.l, "NutSerL_LRo.txt")
...
write_tsv(MineralSer.df, "MineralSer_LRo.txt")
...
write_tsv(MonProdSer_CN.df, "MonProdSerCN_LRo.txt")
...
write_tsv(EnzSer.df, "EnzSer_LRo.txt")
...
write_tsv(EnzSer.df.l, "EnzSerL_LRo.txt")
...
write_tsv(EnzProdSer.df.l, "EnzProdSerL_LRo.txt")
...
UptSer.df <- inner_join(C.UptSer.df, N.UptSer.df) %>% inner_join(P.UptSer.df) %>% 
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(UptSer.df, "UptSer_LRo.txt")
...
write_tsv(N.MonomerUptake.df, "N_MonomerUptake_LRo.txt")
...
Net.CUE.df <- as.data.frame(Net.CUE_Series) %>%  cbind(days = 1:1000) %>%
  pivot_longer(cols = 1:length(Net.CUE_Series), names_to = "Simulation", 
               values_to = "CUE") %>% 
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Net.CUE.df, "NetCUE_LRo.txt")
...
write_tsv(Tot.growth, "Tot_Growth_LRo.txt")
...
write_tsv(Tot.resp, "TotResp_LRo.txt")

Tot.resp.comp <- 
  Resp.comp.df %>% 
  group_by(Simulation, Mechanism, Litter.CN, Source) %>%
  summarize(Tot.resp = sum(Resp, na.rm = TRUE))
write_tsv(Tot.resp.comp, "TotRespComp_LRo.txt")
...
write_tsv(Tot.C.Uptake, "TotCuptake_LRo.txt")
...
write_tsv(Tot.CUE, "TotCUE_LRo.txt")
...
Tot.Biomass <-
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Tot.Biomass, "TotBiomass_LRo.txt")
...
Tot.Enz <- 
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Tot.Enz, "TotEnz_LRo.txt")
...
Tot.Mon <- 
  ...
  separate(Simulation, into = c("Mechanism", "Litter.CN"), sep = "_", remove = FALSE)
write_tsv(Tot.Mon, "TotMon_LRo.txt")
...
write_tsv(Tot.mass.loss, "TotMassLoss_LRo.txt")

Days.to.50 <- 
  Litter.subs.df %>% group_by(Simulation, Mechanism, Litter.CN) %>%
  mutate(Remaining = (Litter.C/max(Litter.C, na.rm = TRUE))*100) %>%
  filter(Remaining > 50) %>% summarize(Days.to.50 = max(days, na.rm = TRUE))
write_tsv(Days.to.50, "DaysTo50_LRo.txt")
...
write_tsv(Tot.excretion, "TotExcretion_LRo.txt")
...
# # Total Biomass
# pdf("Total_Biomass_LRo.pdf", height = 4, width = 5)
# Tot.Biomass %>% mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   ggplot(aes(x = Litter.CN, y = Tot.Biomass/1000000)) +
#   geom_path(aes(color = Mechanism)) +
#   labs(x = "Litter C:N", y = "Total biomass (kg)") +
#   expand_limits(y = 0) +
...
# pdf("Total_Carbon_Growth_LRo.pdf", height = 3, width = 6)
...
# 
# # Total Respiration
# pdf("Total_respiration_LRo.pdf", height = 3, width = 6)
...
# pdf("Total_respiration_per_biomass_LRo.pdf", height = 3, width = 6)
...
# pdf("Overflow_respiration_LRo.pdf", height = 4, width = 5)
# Tot.resp.comp %>% #mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   #filter(Source == "Overflow") %>%
#   ggplot(aes(x = Litter.CN, y = Tot.Overflow)) +
#   geom_path(aes(color = Mechanism)) +
#   labs(y = "Overflow Respiration (mg CO2)", x = "Litter C:N", subtitle = "Leaching = 1") +
#   scale_color_manual(values = palette) +
#   theme_bw() +
...
# pdf("Maintenance_respiration_LRo.pdf", height = 4, width = 5)
# Tot.resp.comp %>% #mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   #filter(Source == "Maint") %>%
#   ggplot(aes(x = Litter.CN, y = Tot.Maint)) +
#   geom_path(aes(color = Mechanism)) +
#   expand_limits(y = 0) +
...
# pdf("Maintenance_per_biomass_LRo.pdf", height = 3, width = 6)
...
# 
# # Growth respiration
# pdf("Growth_respiration_LRo.pdf", height = 4, width = 5)
# Tot.resp.comp %>% #mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   #filter(Source == "Growth") %>%
#   ggplot(aes(x = Litter.CN, y = Tot.Growth)) +
#   geom_path(aes(color = Mechanism)) +
...
# pdf("Growth_respiration_per_biomass_LRo.pdf", height = 3, width = 6)
# Tot.resp.comp %>% left_join(Tot.Biomass) %>%
#   mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   filter(Source == "Growth") %>%
#   ggplot(aes(x = Litter.CN, y = Tot.resp/Tot.Biomass)) +
...
# pdf("CUE_LRo.pdf", height = 4, width = 5)
# Tot.CUE %>% #mutate(Litter.CN = as.numeric(str_extract(Litter.CN, "[0-9]{2}"))) %>%
#   ggplot(aes(x = Litter.CN, y = CUE)) +
#   geom_path(aes(color = Mechanism)) +
#   labs(y = "Carbon-use efficiency", x = "Litter C:N", subtitle = "Leaching = 1") +
#   theme_bw() +
#   scale_color_manual(values = palette) +
...
# pdf("Years_to_50_mass_loss_LRo.pdf", height = 4, width = 5)
...
# pdf("Final_remaining_mass_LRo.pdf", height = 3, width = 6)
...
# 
# # Total excreted NH4
# pdf("Total_NH4_excreted_LRo.pdf", height = 4, width = 5)
# Tot.mineral %>% 
#   ggplot(aes(x = Litter.CN, y = NH4.ex)) +
#   geom_path(aes(color = Mechanism)) +
#   labs(x = "Litter C:N", y = "Total excreted NH4", subtitle = "Leaching = 1") +
#   scale_color_manual(values = palette) +
#   theme_bw() +
#   theme(legend.position = "none",
#         panel.grid = element_blank())
...
# 
# # Total excreted PO4
# pdf("Excreted_PO4_LRo.pdf", height = 3, width = 6)
...
# pdf("Enzyme_CN_LRo.pdf", height = 4, width = 5)
# Tot.EnzProd %>% pivot_wider(names_from = Enzyme, values_from = Tot.enz) %>%
#   ggplot(aes(x = Litter.CN, y = Enz003/Enz002)) +
...
# 
# # Final litter C:N
# pdf("FinalLitterCN_LRo.pdf", height = 4, width = 5)
# Final.Litter %>%
#   ggplot(aes(x = Litter.CN, y = Litter.C/Litter.N)) +
#   geom_line(aes(color = Mechanism)) +
#   geom_abline(slope = 1, linetype = "dashed") +
#   scale_color_manual(values = palette) +
#   labs(x = "Initial litter C:N", y = "Final litter C:N") +
...
# dev.off()
# 
# # Total monomer produced C:N
# pdf("TotMonProdCN_LRo.pdf", height = 4, width = 5)
# Tot.monProdSer.CN %>%
#   ggplot(aes(x = Litter.CN, y = Tot.CN)) +
#   geom_line(aes(color = Mechanism)) +
#   geom_abline(slope = 1, linetype = "dashed") +
#   scale_color_manual(values = palette) +
#   labs(x = "Initial litter C:N", y = "Total monomer produced C:N") +
#   xlim(c(0, 90)) + ylim(c(0, 90)) +
#   theme_bw() +
#   theme(panel.grid = element_blank(),
#         legend.position = c(0.2, 0.7))
...
# 
# # Uptake C:N
# pdf("UptakeCN_LRo.pdf", height = 4, width = 5)
# Tot.uptake %>%
#   ggplot(aes(x = Litter.CN, y = Tot.C.upt/Tot.N.upt)) +
#   geom_line(aes(color = Mechanism)) +
#   geom_abline(slope = 1, linetype = "dashed") +
#   scale_color_manual(values = palette) +
#   ylim(c(0, 90)) + xlim(c(0, 90)) +
#   labs(x = "Litter C:N", y = "Uptake C:N", subtitle = "Leaching = 1") +
#   theme_bw() +
#   theme(legend.position = c(0.2, 0.72),
#         panel.grid = element_blank())
# dev.off()
# 
# # Biomass C:N
# pdf("BiomassCN_LRo.pdf", height = 4, width = 5)
# Biomass.CN %>%
#   ggplot(aes(x = Litter.CN)) +
#   geom_abline(slope = 1, alpha = 0.3) +
#   geom_line(aes(y = max.B.CN, color = Mechanism), linetype = "dashed") +
#   geom_line(aes(y = min.B.CN, color = Mechanism), linetype = "dotted") +
#   geom_line(aes(y = mean.B.CN, color = Mechanism)) +
#   labs(x = "Litter C:N", y = "Biomass C:N") +
#   scale_color_manual(values = palette) +
#   ylim(c(0, 90)) + xlim(c(0, 90)) +
#   theme_bw() +
#   theme(panel.grid = element_blank(),
...
# pdf("NUE_LRo.pdf", height = 4, width = 5)
# Tot.growth %>% 
#   left_join(Tot.mineral) %>% left_join(Tot.uptake) %>% 
#   mutate(NUE = (Tot.N.upt - NH4.ex)/Tot.N.upt, 
#          NUE.growth = (Tot.N.growth)/(Tot.N.upt)) %>%
#   ggplot(aes(x = Litter.CN, y = NUE)) +
#   geom_line(aes(color = Mechanism)) +
#   scale_color_manual(values = palette) +
#   ylim(c(0, 1)) +
#   labs(x = "Litter C:N", subtitle = "Leaching = 1") +
#   theme_bw() +
#   theme(panel.grid = element_blank(), 
#         #legend.position = c(0.2, 0.7),
#         legend.position = "none")
